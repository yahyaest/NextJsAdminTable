...
import { getTableData, handleNestedProperty } from "./services/dataModeling";
...
function AdminPage(props) {
  ...
  const urlSearch = router.asPath.includes("undefined=&")
    ? "?" + router.asPath.split("?")[1].replace("undefined=&", "")
  ...
  const getFilterOptions = (currentFilter) => {
    ...
    result.map((element) => {
      if (currentFilter !== "Filters") {
        const value = handleNestedProperty(element, currentFilter);
    ...
    if (currentTable?.data?.table) {
      ...
      if (searchString && currentTable?.search)
        ...
        data = result.filter(
          ...
          (element) =>
            handleNestedProperty(element, currentFilter) === currentOption
        );
      // 2-sort
